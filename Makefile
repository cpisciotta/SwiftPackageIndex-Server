...
xcbeautify:
	...
	git clone --depth=1 -b charles/GH-Colored https://github.com/cpisciotta/xcbeautify.git .build/checkouts/xcbeautify
...
test-docker:
	...
	docker run --rm -v "$(PWD)":/host -w /host \
	  ...
	  registry.gitlab.com/finestructure/spi-base:0.15.2 \
	  make test
